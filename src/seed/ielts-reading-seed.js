/**
 * Seeds IELTS Reading tests from exercises/ielts/reading/*.json into MongoDB.
 *
 * Run with: npm run seed:reading
 */
import fs from "fs"
...
async function seedReadings() {
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    if (!fs.existsSync(filePath)) {
      console.warn(`[seed:reading] skip missing file: ${item.file}`)
      continue
    }
  ...
  console.log(`[seed:reading] upserted ${written} reading test(s)`)
}
